...
void main(void) {
    ...
    // screenPos = (gl_FragCoord.xy / (vec2(2134, 1600) * 1.0)) * 2.0 - 1.0;
    // screenPos = gl_FragCoord.xy / (resolution * pixel_ratio);
    // screenPos = gl_FragCoord.xy / (vec2(2134, 1600) * 1.0);
...
AFRAME.registerComponent("screen-quad", {
    ...
    */
    ...
    tick: function () {
        if (this.camera_set === false)
        {
            console.log("Screen quad initialization...");
            ...
            this.el.object3DMap["mesh"].material.uniforms.cameraPosition.value = camera.position;
            this.el.object3DMap["mesh"].material.uniforms.cameraWorldMatrix.value = camera.matrixWorld;
            this.el.object3DMap["mesh"].material.uniforms.cameraProjectionMatrixInverse.value.getInverse(camera.projectionMatrix);
...
function onWindowResize() {
    ...
    */
    ...
    // screen_quad.material.uniforms.resolution.value.set(window.innerWidth, window.innerHeight);
